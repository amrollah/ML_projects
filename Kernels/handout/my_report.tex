...
%The \textit{stepwisefit} function has two parameters which we didn't change them from default. They are\textit{penter} and \textit{premove}. We used manual try and failure approach to select best value for these two parameters. Finally we realized that default values produce the best result for our case.
